...
void* StackAllocator::allocate(const size_t size, const size_t alignment) {
  const size_t adjustment = pointer_math::alignForwardAdjustmentWithHeader(
...
void StackAllocator::deallocate(void* p) {
  ...
  const auto* allocationHeader = static_cast<AllocationHeader*>(
    pointer_math::subtract(p, sizeof(AllocationHeader)));

  // Check if the deallocation is in reverse order
  assert(pointer_math::subtract(m_currentPos, allocationHeader->size) != p &&
